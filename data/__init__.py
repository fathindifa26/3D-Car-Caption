...
from data.car3d_dataset import car3d_train, car3d_caption_eval, car3d_retrieval_eval 
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        return train_dataset, val_dataset, test_dataset   
    elif dataset=='caption_car3d':           
        train_dataset = car3d_train(transform_train, config['image_root'], config['ann_root'], max_words=config.get('max_length', 30), prompt=config.get('prompt', ''))
        val_dataset = car3d_caption_eval(transform_test, config['image_root'], config['ann_root'], 'val')        
        test_dataset = car3d_caption_eval(transform_test, config['image_root'], config['ann_root'], 'test')   
        return train_dataset, val_dataset, test_dataset    
